...
Now that we've got a sense of what is going on in this datset, we can try out some of the machine learning visualization tools that come with Yellowbrick. As I mentioned in the introduction, we'll use the `reticulate` package to include Python tools into our R workflow.
...
*NOTE:* I'm a little puzzled as to why I need to use `(1-d['two_year_recid'])` instead of just `d['two_year_recid']`. It seems like high values of two_year_recid (1 rather than 0) should correlate with high values of decile_score. This probably has something to do with the way that YB is ordering the classes -- y=0 is being associated with the second column of X rather than the first. Should fix this before publishing.
...
def race_tv(racestr):
    ...
    y = 1-d['two_year_recid'][race_sel].as_matrix()
...
Here, the "precision" curve is higher and flatter, and crosses the "recall" curve at a higher value of the threshold. The precision curve increases monotonically with the threshold (as the number of false positives decreases) and the recall curve decreases monotonically (as the number of false negatives increases). 
...
A simple analysis like this can't uncover all of the problems with algorithmic decision-making in the criminal justice system, let alone propose a set of truly viable solutions. Hopefully, though, it's clear that simple visualization tools are able to uncover systemic bias and suggest some of the implications of different error measures. The real challenge is convincing goverments and communities to be more skeptical users and consumers of these tools.
